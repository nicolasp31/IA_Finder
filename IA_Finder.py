...
PALABRAS_POSIBLES_IA =[
    ...
    ,"2013:12:23 23:15:00Z","12.0000","WeasyPrint 65.1","PyFPDF 1.7.2 http://pyfpdf.googlecode.com/","Skia/PDF m83","Chromium","Skia/PDF m127"
    ,"jsPDF 2.5.1","generated using python-pptx","2013:01:27 09:14:16Z", "Microsoft Macintosh PowerPoint", "2023:08:21 21:53:30Z","Unknown Creator"
    ,"LibreOffice/25.2.4.3$Linux_X86_64","0000:00:00 00:00:00","Lavf58.20.100","Lavf57.56.101","Lavf58.45.100","Lavf61.1.100","Lavf58.20.100"
...
PALABRAS_CLAVE_MODELO = [
    "","",""
]

PALABRAS_CLAVE_APLICACION =[
...
def verificar_archivo_ia(metadata):
    """
    Busca en la metadata hasta las primeras 3 etiquetas con palabras clave que indiquen posible generación con IA,
    evitando repetir la misma etiqueta varias veces.
    Si no encuentra en la lista principal, busca en lista secundaria de posibles metadatos sospechosos.
    Retorna (bool, mensaje con las coincidencias encontradas o aviso de sospecha).
    """
    coincidencias = []
    etiquetas_detectadas = set()
    # Asumiendo que etiquetas_excluidas, PALABRAS_CLAVE_IA y PALABRAS_POSIBLES_IA están definidos en el scope global
    ...
    # Si no hay coincidencias en lista principal, buscar en PALABRAS_POSIBLES_IA
...
class HiloMetadata(QThread):
    ...
    terminado = pyqtSignal(dict, object, str)  # metadata, es_ia, mensaje
...
class DetectorImagenIA(QWidget):
    ...
    def finalizar_analisis(self, metadata, es_ia, mensaje):
        ...
        if es_ia:
            texto = "<b>Resultado de analisis:</b> Archivo generado por IA\n"
        elif es_ia is None:
            self.icono_resultado.setText("⚠️")  # Icono de advertencia o intermedio
            texto = "<b>Resultado de analisis:</b> Posible generación por IA (no confirmado)\n"
        else:
            texto = "<b>Resultado de analisis:</b> Archivo genuino<br><b>0% IA detectada</b>\n"
